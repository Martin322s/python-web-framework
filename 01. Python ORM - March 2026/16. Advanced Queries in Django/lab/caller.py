...
def product_quantity_ordered():
    ...
    for product in products:
        result.append(
            f"Quantity ordered of {product.name}: {product.total_quantity}"
        )
...
def filter_products():
    products = (
        Product.objects
        .filter(is_available=True, price__gt=Decimal("3.00"))
        .order_by("-price", "name")
    )
...
def give_discount():
    Product.objects.filter(
        is_available=True,
        price__gt=Decimal("3.00")
    ).update(price=F("price") * Decimal("0.70"))
