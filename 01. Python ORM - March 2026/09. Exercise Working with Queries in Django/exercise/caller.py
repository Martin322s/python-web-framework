...
def update_operation_systems():
    ...
    for laptop in laptops:
        if laptop.brand == 'Asus':
            ...
            laptop.operation_system = 'Mac OS'
...
def grand_chess_title_IM():
    ChessPlayer.objects.filter(rating__gte=2300).filter(rating__lte=2399).update(title='IM')

def grand_chess_title_FM():
    ChessPlayer.objects.filter(rating__gte=2200).filter(rating__lte=2299).update(title='FM')

def grand_chess_title_regular_player():
    ChessPlayer.objects.filter(rating__gte=0).filter(rating__lte=2199).update(title='regular player')
...
def show_hard_dungeons() -> str:
    hard = Dungeon.objects.filter(difficulty="Hard").order_by("-location")
    return "\n".join(
