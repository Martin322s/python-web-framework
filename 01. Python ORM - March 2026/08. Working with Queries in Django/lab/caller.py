...
def find_authors_nationalities():
    ...
    result = [f"{author.first_name} {author.last_name} is {author.nationality}" for author in authors]
    return "\n".join(result)

def order_books_by_year():
    books = Book.objects.all().order_by('publication_year', 'title')
    result = [f"{x.publication_year} year: {x.title} by {x.author}" for x in books]
    return "\n".join(result)
...
def filter_authors_by_nationalities(nationality):
    ...
    for author in authors:
        if author.biography:
            result += f"{author.biography}\n"
        else:
            result += f"{author.first_name} {author.last_name}\n"
