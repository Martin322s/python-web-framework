...
class VideoGameManager(models.Manager):
    ...
    def highest_rated_game(self):
        return self.order_by('-rating', 'title').first()
    ...
    def clean(self):
        if not (0.0 <= self.rating <= 10.0):
            raise ValidationError({
                'rating': "The rating must be between 0.0 and 10.0"
            })

        if not (1990 <= self.release_year <= 2023):
            raise ValidationError({
                'release_year': "The release year must be between 1990 and 2023"
            })
...
class Invoice(models.Model):
    ...
    def get_invoice_with_billing_info(invoice_number: str):
        return Invoice.objects.select_related('billing_info').get(
...
class Project(models.Model):
    ...
    def get_programmers_with_technologies(self):
        return Programmer.objects.filter(
            projects=self
...
class Programmer(models.Model):
    ...
    def get_projects_with_technologies(self):
        return self.projects.all().prefetch_related('technologies_used')
